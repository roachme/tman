...
---Init core modules.
function core.init()
    local unit_dir = struct.units.path
    local task_dir = struct.tasks.path
...
function core.add(envname, id, units)
    ...
    local envdesc = "autogenerated environment description"
    ...
    elseif not taskenv.exist(envname) then
        if not taskenv.check(envname) then
            core.die(1, "illegal environment name", "env")
            return false
        elseif not taskenv.add(envname, envdesc) then
